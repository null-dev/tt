...
impl fmt::Display for OsError {
    fn fmt(&self, _f: &mut fmt::Formatter<'_>) -> Result<(), fmt::Error> {
        match *self {
            ...
            #[cfg(feature = "fbdev")]
            OsError::FbDevError(ref e) => _f.pad(e),
            #[cfg(feature = "fbdev")]
            OsError::FbDevMisc(e) => _f.pad(e),
        }
    }
}
...
impl Window {
    ...
    pub fn new<T>(
        ...
        match *window_target {
            ...
            #[cfg(feature = "fbdev")]
            EventLoopWindowTarget::FbDev(ref window_target) => {
                fbdev::Window::new(window_target, attribs, pl_attribs).map(Window::FbDev)
            }
        }
    }
    ...
    pub fn current_monitor(&self) -> Option<RootMonitorHandle> {
        match self {
            ...
            #[cfg(feature = "fbdev")]
            Window::FbDev(ref window) => {
                let current_monitor = MonitorHandle::FbDev(window.current_monitor()?);
                Some(RootMonitorHandle {
                    inner: current_monitor,
                })
            }
    ...
    pub fn available_monitors(&self) -> VecDeque<MonitorHandle> {
        match self {
            ...
            Window::Kms(ref window) => window
                ...
                .collect(),
            #[cfg(feature = "fbdev")]
            Window::FbDev(ref window) => window
                .available_monitors()
                .into_iter()
                .map(MonitorHandle::FbDev)
                .collect(),
        }
    }
    ...
    pub fn primary_monitor(&self) -> Option<RootMonitorHandle> {
        match self {
            ...
            #[cfg(feature = "fbdev")]
            Window::FbDev(ref window) => window.primary_monitor(),
        }
    }
    ...
    pub fn raw_window_handle(&self) -> RawWindowHandle {
        match self {
            ...
            #[cfg(feature = "fbdev")]
            Window::FbDev(ref window) => RawWindowHandle::Drm(window.raw_window_handle()),
        }
    }
}
...
impl<T: 'static> EventLoop<T> {
    pub(crate) fn new(attributes: &PlatformSpecificEventLoopAttributes) -> Self {
        ...
        #[cfg(feature = "fbdev")]
        if attributes.forced_backend == Some(Backend::FbDev) {
            // TODO: Propagate
            return EventLoop::new_fbdev_any_thread().expect("failed to open fbdev connection");
        }
        ...
        if let Ok(env_var) = env::var(BACKEND_PREFERENCE_ENV_VAR) {
            match env_var.as_str() {
                ...
                "fbdev" => {
                    #[cfg(feature = "fbdev")]
                    return EventLoop::new_fbdev_any_thread()
                        .expect("Failed to initialize fbdev backend");
                    #[cfg(not(feature = "fbdev"))]
                    panic!("fbdev feature is not enabled");
                }
                _ => panic!(
        ...
        #[cfg(feature = "fbdev")]
        let fbdev_err = match EventLoop::new_fbdev_any_thread() {
            Ok(event_loop) => return event_loop,
            Err(err) => err,
        };
        ...
        #[cfg(not(feature = "fbdev"))]
        let fbdev_err = "backend disabled";

        panic!(
            "Failed to initialize any backend! Wayland status: {:?} X11 status: {:?} DRM status: {:?} FBDev status: {:?}",
            wayland_err, x11_err, drm_err, fbdev_err
            );
    }
    ...
    #[cfg(feature = "fbdev")]
    fn new_fbdev_any_thread() -> Result<EventLoop<T>, RootOsError> {
        fbdev::EventLoop::new().map(EventLoop::FbDev)
    }
...
pub enum EventLoopWindowTarget<T> {
    ...
    #[cfg(feature = "fbdev")]
    FbDev(fbdev::EventLoopWindowTarget<T>),
}
...
impl<T> EventLoopWindowTarget<T> {
    ...
    pub fn available_monitors(&self) -> VecDeque<MonitorHandle> {
        match *self {
            ...
            EventLoopWindowTarget::Kms(ref evlp) => evlp
                ...
                .collect(),
            #[cfg(feature = "fbdev")]
            EventLoopWindowTarget::FbDev(ref evlp) => evlp
                .available_monitors()
                .into_iter()
                .map(MonitorHandle::FbDev)
                .collect(),
    ...
    pub fn primary_monitor(&self) -> Option<RootMonitorHandle> {
        match *self {
            ...
            #[cfg(feature = "fbdev")]
            EventLoopWindowTarget::FbDev(ref evlp) => evlp.primary_monitor(),
        }
    }
}
